...
myWebApp.controller("cartHeaderController", function ($scope) {
    $scope.cart_nums = JSON.parse(localStorage.getItem('buy-cart')).length;
    // console.log($scope.cart_nums);
    $scope.cartRedirectPag = function() {
        if ($scope.cart_nums == 0) {
            $window.location.href = '#/emptycar';
        } else {
